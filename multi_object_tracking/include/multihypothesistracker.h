...
namespace MultiHypothesisTracker {
  ...
  double get_time_high_res ();
	...
	// can be applied to filter out hypotheses during correction and prediction
	// default: return all hypotheses
	class HypothesisFilter {
	public:
		HypothesisFilter() {}
		~HypothesisFilter() {}

		virtual bool passthrough( Hypothesis* hypothesis );
		virtual std::vector< Hypothesis* > filter( const std::vector< Hypothesis* >& hypotheses );
	};
	...
	public:
		...
		virtual void predict( double dt, const vnl_vector< double >& control, HypothesisFilter* filter = new HypothesisFilter() );
		// returns vector of assignments
		virtual std::vector< unsigned int > correct( const std::vector< vnl_vector< double > >& measurements, HypothesisFilter* filter = 0);
		virtual std::vector< unsigned int > correctAmbiguous( const std::vector< Measurement >& measurements, bool createNewHypotheses = true, HypothesisFilter* filter = 0 );

		virtual std::vector< unsigned int > correctAmbiguous_simplified( const std::vector< Measurement >& measurements, bool createNewHypotheses = true, HypothesisFilter* filter = 0 );
		std::vector< unsigned int > correct_hungarian_simplified( const std::vector< Measurement >& measurements, HypothesisFilter* filter );
...
// 		inline void unlockHypotheses() { m_hypothesisMutex.unlock(); }
		void clear();
    void deleteSpuriosHypotheses( HypothesisFilter* filter = new HypothesisFilter() ); // <- Mutex has to be locked!
