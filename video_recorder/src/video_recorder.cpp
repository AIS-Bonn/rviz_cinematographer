...
void VideoRecorderNodelet::onInit()
{
  ...
  rendering_finished_sub_ = nh_.subscribe("/rviz/finished_rendering_trajectory", 1, &VideoRecorderNodelet::renderingFinishedCallback, this);
...
{
  NODELET_ERROR("got record params");
  ...
  NODELET_INFO_STREAM("path to video is " << path_to_output_);
  ...
  if(!process_images_thread_)
    process_images_thread_ = boost::shared_ptr<boost::thread>(new boost::thread(boost::bind(&VideoRecorderNodelet::processImages, this)));
}

void VideoRecorderNodelet::renderingFinishedCallback(const rviz_cinematographer_msgs::Finished::ConstPtr& finished)
{
  NODELET_ERROR("rendering finished callback ");

  ros::Rate r(30); // 30Hz
  if(finished->is_finished)
  {
    NODELET_ERROR("rendering finished callback - within if statement");
    ...
    record_finished_pub_.publish(finished);
    NODELET_ERROR("rendering finished callback - end of if statement");
...
void VideoRecorderNodelet::imageCallback(const sensor_msgs::ImageConstPtr& input_image)
{
  ...
  image_queue_.push(cv_ptr);
  
  if(image_queue_.size() >= max_queue_size_)
  {
    NODELET_ERROR("max queue size exceeded");
    ...
    wait_duration_msg.seconds = wait_duration.toSec();
...
void VideoRecorderNodelet::processImages()
{
  ...
  while(ros::ok())
  {
    if(!image_queue_.empty())
    {
      ...
      NODELET_ERROR_STREAM("image size cols " << cv_ptr->image.cols << " " << cv_ptr->image.rows);
      ...
      NODELET_ERROR_STREAM("queue size " << (int)image_queue_.size());
...
void VideoRecorderNodelet::resizeWatermark(cv::Mat& watermark, int image_width)
{
...
void VideoRecorderNodelet::addWatermark(cv::Mat& image, cv::Mat& watermark)
{
  ...
  for(int watermark_row = 0; watermark_row < watermark.rows; watermark_row++, image_row++)
  {
    ...
    for(int watermark_col = 0; watermark_col < watermark.cols; watermark_col++, image_col++)
    {
      ...
      if(pixel_alpha != 0)
        for(int i = 0; i < 3; ++i)
          image.at<cv::Vec3b>(image_row, image_col)[i] = cv::saturate_cast<uchar>(alpha * image.at<cv::Vec3b>(image_row, image_col)[i] + (1.f-alpha) * watermark.at<cv::Vec4b>(watermark_row, watermark_col)[i]);
